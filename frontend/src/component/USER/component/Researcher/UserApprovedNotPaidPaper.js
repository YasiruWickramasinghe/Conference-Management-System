...
export default class Home extends Component {
  ...
  retrievePapers = () => {
    ...
    axios
      .get(`http://localhost:5050/rpaper/approvednotpaidpapers/${decode.email}`)
      .then((res) => {
        if (res.data.success) {
          this.setState({
  ...
  handleSearchArea = (e) => {
    ...
    axios.get(`http://localhost:5050/rpaper/approvednotpaidpapers/${decode.email}`).then((res) => {
      if (res.data.success) {
  ...
  render() {
    return (
      <div className="container" style={{ marginTop: "5rem" }}>
        <div class="d-flex">
          <div class="mr-auto p-2"> <h4 style={{fontWeight:"bold"}}>Pending Paid Research Papers</h4></div>
          <div class="p-2"><input
